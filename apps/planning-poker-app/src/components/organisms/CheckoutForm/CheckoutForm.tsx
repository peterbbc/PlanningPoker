...
import { StripeCardElement } from '../../molecules/StripeCardElement/StripeCardElement';

interface CheckoutFormProps {
  onChange: (event: StripeCardElementChangeEvent) => any;
  isDisabled?: boolean;
  isErrorMessage?: false | string;
}
...
export const CheckoutForm = ({
  ...
  return (
    <>
      ...
      <StripeCardElement isError={!!isErrorMessage} onChange={onChange} />
      {isErrorMessage && (
        <>
          <VerticalSpacing spacing="spacing-xs" />
          <Paragraph color="danger">{isErrorMessage}</Paragraph>
        </>
      )}
    </>
  );
